...
# 2. 定义直线绘制函数
# -------------------------------
def draw_line_back_forth(pandaId, start_pos, end_pos, steps=200, cycles=5):
    line_ids = []  # 用于存储调试线ID
    t_step = 0
    ...
    for cycle in range(cycles):
        for t in range(steps + 1):
            ...
            joint_poses = p.calculateInverseKinematics(
                ...
                lowerLimits=[l[0] for l in joint_limits],
                upperLimits=[l[1] for l in joint_limits],
                restPoses=[0]*7
            )
            ...
            # 画轨迹（调试线）
            if t_step > 0:
                line_id = p.addUserDebugLine(prev_p, curr_p, [1, 0, 0], 2)
...
try:
    #调用移动函数
    draw_line_back_forth(pandaId, start, end, steps=200, cycles=5)
    while True:
       # draw_line_joint_space(pandaId, start, end)
