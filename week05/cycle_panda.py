...
ctrl_z = p.addUserDebugParameter("Target_Z", 0.65, 1.2, 1.2)
...
try:
    ...
    while True:
        ...
        if run_ik > 0.5:
            ...
            # 圆参数
            # 从滑块读取圆心（推荐）
            cx = p.readUserDebugParameter(ctrl_x)
            cy = p.readUserDebugParameter(ctrl_y)
            cz = p.readUserDebugParameter(ctrl_z)
           ...
           # 圆参数
            ...
            # 圆轨迹（XY 平面）
            tx = cx + radius * math.cos(t)
            ty = cy + radius * math.sin(t)
            tz = cz
